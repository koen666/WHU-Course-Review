...
void RadixSort1(LinkList<int>& L, int d, int r) {
    ...
    LinkNode<int>* front[MAXR]; // 存放链队头指针的数组
    LinkNode<int>* rear[MAXR];  // 存放链队尾指针的数组
    LinkNode<int> *p, *t;

    for (int i = 0; i < d; i++) { // 从低位到高位循环 d 趟 
        // --- 分配阶段 ---
        for (int j = 0; j < r; j++) // 初始化各链队 
            front[j] = rear[j] = nullptr;

        p = L.head->next; // p 指向链表第一个数据节点
        while (p != nullptr) {
            int k = geti(p->data, r, i); // 提取关键字第 i 位的值 k 
            
            if (front[k] == nullptr) { // 队列为空，建立队头 
                front[k] = p;
                rear[k] = p;
            } else { // 队列不为空，采用尾插法 
                rear[k]->next = p;
        ...
        // --- 收集阶段 ---
        LinkNode<int>* h = nullptr; // 用于收集所有节点的新链表头
        for (int j = 0; j < r; j++) { // 按基数顺序遍历队列 
            if (front[j] != nullptr) {
                if (h == nullptr) { // 找到第一个非空队列 
                    h = front[j];
                    t = rear[j];
                } else { // 将后续非空队列接在前一个队列尾部 
                    t->next = front[j];
                    t = rear[j];
                }
            }
        }
        t->next = nullptr; // 收集完毕，尾节点置空 
